...
__launch_bounds__(SORT_BLOCKDIM)
    __global__ void sort(vec2d_t<uint32_t> inouts, size_t len, uint32_t win,
                         ...
                         uint32_t wbits, uint32_t lsbits0, uint32_t lsbits1, uint32_t *d_pidx);
...
__device__ __forceinline__ void upper_count_digits(const uint32_t src[], uint32_t base, uint32_t len,
...
__device__ __forceinline__ void upper_scatter(uint2 dst[], const uint32_t src[], uint32_t base, uint32_t len,
                                              uint32_t lshift, uint32_t rshift, uint32_t mask,
...
__device__ static void upper_sort(uint2 dst[], const uint32_t src[], uint32_t len,
                                  ...
                                  uint32_t histogram[], uint32_t pidx[] = nullptr)
{
    ...
    upper_count_digits(src, base, slice, lshift, digit, mask);
...
#pragma unroll
    ...
    upper_scatter(dst, src, base, slice, lshift, digit, mask, pidx);
...
__device__ __forceinline__ void lower_count_digits(const uint2 src[], uint32_t len, uint32_t mask)
{
    zero_counters();
...
__device__ __forceinline__ void lower_scatter(uint32_t dst[], const uint2 src[], uint32_t len, uint32_t mask)
{
#pragma unroll 1 // the subroutine is memory-io-bound, unrolling makes no difference
    for (uint32_t i = threadIdx.x; i < len; i += SORT_BLOCKDIM)
    {
...
__device__ __noinline__ static void lower_sort(uint32_t dst[], const uint2 src[],
                                               uint32_t base, uint32_t len, uint32_t histogram[],
                                               uint32_t bits = DIGIT_BITS)
{
    const uint32_t mask = (1 << bits) - 1;

    lower_count_digits(src += base, len, mask);
...
#pragma unroll
    ...
    lower_scatter(dst, src, len, mask);

    __syncthreads();
}

__device__ __forceinline__ void sort_row(uint32_t inout[], size_t len, uint2 temp[],
