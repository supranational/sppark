...
template <class point_t, class bucket_t> static void integrate_buckets(point_t &out, bucket_t buckets[], size_t wbits) {
    bucket_t ret, acc;
...
static void bucket(bucket_t buckets[], size_t booth_idx, size_t wbits, const affine_t &p) {
    booth_idx &= (1 << wbits) - 1;
    if (booth_idx--)
        buckets[booth_idx].add(p);
}

template <class bucket_t> static void prefetch(const bucket_t buckets[], size_t booth_idx, size_t wbits) {
#if 0
    booth_idx &= (1<<wbits) - 1;
    if (booth_idx--)
...
static void tile(point_t &ret, const affine_t points[], size_t npoints, const unsigned char *scalars, size_t nbits,
...
template <typename T> static size_t num_bits(T l) {
...
std::tuple<size_t, size_t, size_t> static breakdown(size_t nbits, size_t window, size_t ncpus) {
    size_t nx, ny, wnd;
...
static void mult(point_t &ret, const affine_t &point, const pow_t scalar, size_t top) {
    ret.inf();
    if (point.is_inf())
        return;

    struct is_bit {
        static bool set(const pow_t v, size_t i) { return (v[i / 8] >> (i % 8)) & 1; }
    };
...
template <class bucket_t, class point_t, class scalar_t, class affine_t = class bucket_t::affine_t>
static void mult_pippenger(point_t &ret, const affine_t points[], size_t npoints, const scalar_t _scalars[], bool mont,
    ...
    if (mont) {
        ...
        if (ncpus < 2 || npoints < 1024) {
            ...
            da_pool->par_map(npoints, 512, [&](size_t i) { _scalars[i].to_scalar(store[i]); });
        }
        scalars = &store[0];
    }
    ...
    if (ncpus < 2 || npoints < 32) {
        ...
        while (bit0 -= wbits) {
            tile(p, points, npoints, scalars[0], nbits, &buckets[0], bit0, wbits, cbits);
            ret.add(p);
            for (size_t i = 0; i < window; i++)
                ret.dbl();
            cbits = wbits = window;
        }
        tile(p, points, npoints, scalars[0], nbits, &buckets[0], 0, wbits, cbits);
        ret.add(p);
        return;
    }
    ...
    size_t dx = npoints / nx, y = window * (ny - 1);
    ...
    while (n_workers--) {
        da_pool->spawn([&, window, total, nbits, nx, counter]() {
            ...
            if ((work = counter++) < total) {
                ...
                do {
                    size_t x = grid[work].x, dx = grid[work].dx, y = grid[work].y, dy = grid[work].dy;
                    tile(grid[work].p, &points[x], dx, scalars[x], nbits, &buckets[0], y, dy, dy + (dy < window));
...
template <class bucket_t, class point_t, class scalar_t, class affine_t = class bucket_t::affine_t>
static void mult_pippenger(point_t &ret, const std::vector<affine_t> &points, const std::vector<scalar_t> &scalars,
                           bool mont, thread_pool_t *da_pool = nullptr) {
    mult_pippenger<bucket_t>(ret, points.data(), std::min(points.size(), scalars.size()), scalars.data(), mont,
                             da_pool);
}
...
template <class bucket_t, class point_t, class scalar_t, class affine_t = class bucket_t::affine_t>
static void mult_pippenger(point_t &ret, slice_t<affine_t> points, slice_t<scalar_t> scalars, bool mont,
                           thread_pool_t *da_pool = nullptr) {
    mult_pippenger<bucket_t>(ret, points.data(), std::min(points.size(), scalars.size()), scalars.data(), mont,
                             da_pool);
}
