                       ...
                       const int stage, const int iterations)
{
    ...
    #pragma unroll
    for (int z = 0; z < z_count; z++) {
        xchg[z][x] = inout[offset + x + (z << (stage + 1))];
    }
...
template<unsigned int z_count>
__launch_bounds__(768, 1) __global__
void _CT_NTT_3D(const unsigned int radix, const unsigned int lg_domain_size,
                const unsigned int stage, const unsigned int iterations,
                fr_t* d_inout, const fr_t (*d_partial_twiddles)[WINDOW_SIZE],
                const fr_t* d_radix6_twiddles, const fr_t* d_radixX_twiddles,
                bool is_intt, const fr_t d_domain_size_inverse)
...
template __global__
void _CT_NTT_3D<Z_COUNT>(unsigned int, unsigned int, unsigned int, unsigned int,
                         fr_t*, const fr_t (*)[WINDOW_SIZE], const fr_t*, const fr_t*,
                         bool, const fr_t);
...
public:
    ...
    void step(int iterations)
    {
        ...
        if (num_blocks < Z_COUNT)
            ...
            _CT_NTT_3D<Z_COUNT><<<num_blocks/Z_COUNT, block_size, Z_COUNT*shared_sz, stream>>>(NTT_ARGUMENTS);
